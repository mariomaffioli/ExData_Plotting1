...
par(mfrow = c(2, 2))
with(df2, {
    plot(RDate, Global_active_power, type="n", xlab="",ylab="Global Active Power (kilowatts)" ) 
    lines(RDate,Global_active_power )
    
    plot(RDate, Voltage, type="n", xlab="",ylab="Voltage" )
    lines(RDate,Voltage )
    
    plot(RDate, Sub_metering_1, type="n", xlab="",ylab="Energy Sub metering", col="black" ) 
    lines(RDate,Sub_metering_1, col="black")
    lines(RDate,Sub_metering_2, col="red" )
    lines(RDate,Sub_metering_3, col="blue")
    legend("topright", c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),lty=c(1,1),col=c("black","blue","red"),bty="n" )
    
    plot(RDate, Global_reactive_power, type="n", xlab="datetime") 
    lines(RDate,Global_reactive_power )
